...
Wpisz `false` aby wyłączyć end, lub `true` żeby włączyć.
...
Wpisz `false` aby wyłączyć informację o przeciążeniu, lub `true` żeby włączyć.
...
Wpisz `false`, jeżeli chcesz, żeby gra zachowywała się standardowo, lub `true`, żeby gracz którego miejsce wylogowania zostało zabudowane blokami po prostu się zaczął w nich dusić.
...
Wpisz `false` aby wyłączyć profilowanie, lub `true` aby je włączyć.
...
Wpisz `false` aby wyłączyć wyświetlanie pluginów, lub `true` aby je włączyć.
...
Wpisz `false` aby całkowicie wyłączyć ostrzeżenia, `default` żeby włączyć ostrzeżenia których twórca pluginu nie wyciszył, lub `true` aby włączyć wszystkie ostrzeżenia.
